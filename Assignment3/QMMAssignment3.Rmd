...
get.sensitivity.rhs(x)$duals[1] # the zero shadow price indicates that the profit will not be changed and the total profit stand at 696000 $
```
...
```
```{r}
solve(dual) # '0' optimal solution exists
...
get.sensitivity.rhs(dual)#  shadow price of the dual are close to the variables in primal. Since there is a difference in Y10 and Y11 are non existing in primal there is an observed difference.
